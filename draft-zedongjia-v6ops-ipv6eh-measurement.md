...
IPv6 includes Extension Headers (EHs) {{RFC7045}} {{RFC8200}}. Located between
...
As EHs are increasingly adopted, characterizing their traversal behavior
...
A comprehensive understanding of EH reachability across a broad range of
...
EH processing also introduces security challenges {{RFC9098}} {{RFC9099}}.
...
{{Atlasis2016}} {{RFC7112}} {{FragEvasion}}. Moreover, specific EH types
are associated with known architectural concerns that can be used for
targeted attacks, such as amplification {{RFC5095}}, overlapping fragment
evasion {{RFC5722}}, processing of atomic fragments {{RFC6946}},
...
- **EH Path Traversal Capability:** Specific EHs, in particular the
...
- **Fragment header:** Used for IPv6 fragmentation when
...
**Availability-oriented ossification:** In industry categories such as Travel,
...
**Security-oriented ossification:** The Government category exhibits a
...
- **MH, HIP, and Shim6** --- which are not defined in {{RFC8200}} ---
  ...
  leaving security blind spots.
- The spatial distribution of apparently evaded firewalls further
...
levels, particularly under TCP/UDP.
...
intermediate ASes** (71.8% and 77.1% under TCP/22 respectively), which may
...
The measurements indicate widespread potential EH-based firewall evasion
